...
void *read_data(void *params){
    while ((read_size = recv(client_sock, &message, QNT_DEVICES * sizeof(int), 0)) > 0) { 
        printf("\nVALOR: %d\n", message[0]);
...
void send_data(int code, double value){
    ...
    send(client_sock, &dataClient, 2 * sizeof(double), 0);
}
...
void *escuta(void *params){
    ...
    int read_size;
    while((read_size = recv(client_sock, &server_reply, 2 * sizeof(double), 0) > 0)) {
        // printf("ESCUTA\n");
        printf("TENTOU LER");
        if(read_size < 0) { 
            puts("recv failed"); 
