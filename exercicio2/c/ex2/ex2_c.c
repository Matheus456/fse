 ...
 *
 */
void print_sensor_data(struct bme280_data *comp_data, float *total);
...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    int repeticoes = 0;
    float total[3] = {0, 0, 0}; //[0] -> temp, [1] -> press, [2] -> hum
    while (1)
    {
        ...
        print_sensor_data(&comp_data, total);
        sleep(1);
        repeticoes++;
        if (repeticoes == 10)
        {
            register_csv(total); //Registrar data, hora e dados médios em csv.
            total[0] = 0;
            total[1] = 0;
            total[2] = 0;
            repeticoes = 0;
